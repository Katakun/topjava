<%@ page contentType="text/html;charset=UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<html lang="ru">
<head>
    <link rel="stylesheet" href="css/style.css">
    <title>Meals</title>
</head>
<body>
<h3><a href="index.html">Home</a></h3>
<hr>
<h2>Meals</h2>
<p><a href="meals?action=add">Add meal</a></p>
<table class="table">
    <tr>
        <th>Date</th>
        <th>Description</th>
        <th>Calories</th>
        <th></th>
        <th></th>
    </tr>
    <c:forEach items="${mealToList}" var="mealsTo">
        <jsp:useBean id="mealsTo" type="ru.javawebinar.topjava.model.MealTo"/>
        <c:set value="${mealsTo.excess ? 'red' : 'green'}" var="color"/>
        <tr style="color:${color}">
            <td>${mealsTo.date} ${mealsTo.time}</td>
            <td>${mealsTo.description}</td>
            <td>${mealsTo.calories}</td>
            <td><a href="meals?id=${mealsTo.id}&action=edit">Update</a></td>
            <td><a href="meals?id=${mealsTo.id}&action=delete">Delete</a></td>
        </tr>
    </c:forEach>
</table>
</body>
</html>
